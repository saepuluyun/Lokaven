package com.aniqma.lokaventour.host.modul.activity.tourmanagement.detailtourpackage.tourpackagedetailprivate;
...
import com.aniqma.lokaventour.host.modul.activity.tourmanagement.detailtourpackage.tourpackagedetailprivate.dialog.DialogRefundPolicy;
import com.aniqma.lokaventour.host.modul.activity.tourmanagement.detailtourpackage.tourpackagedetailprivate.adapter.AddonsPrivateTripPackageAdapter;
import com.aniqma.lokaventour.host.modul.activity.tourmanagement.detailtourpackage.tourpackagedetailprivate.adapter.DiscussionAdapter;
import com.aniqma.lokaventour.host.modul.activity.tourmanagement.detailtourpackage.tourpackagedetailprivate.adapter.PoliciesPrivateTripPackageAdapter;
import com.aniqma.lokaventour.host.modul.activity.tourmanagement.detailtourpackage.tourpackagedetailprivate.adapter.PriceAdapter;
import com.aniqma.lokaventour.host.modul.activity.tourmanagement.detailtourpackage.tourpackagedetailprivate.adapter.SchedulesPrivateTripDetailAdapter;
import com.aniqma.lokaventour.host.modul.activity.tourmanagement.detailtourpackage.tourpackagedetailprivate.adapter.TagsAdapter;
import com.aniqma.lokaventour.host.modul.activity.tourmanagement.detailtourpackage.tourpackagedetailprivate.adapter.TourPackageImageAdapter;
...
import static com.aniqma.lokaventour.host.helper.arraylist.SortingListHelper.findMin;
import static com.aniqma.lokaventour.host.modul.activity.general.home.HomeActivity.RESULT_DELETE;

public class PrivateTripPackageDetailActivity extends AppCompatActivity implements
        View.OnClickListener, IViewDetailTour, IViewGetNewestDiscussionFromTourIdService, iViewDeleteTour, IViewGetNewestReview,
        IViewCheckToken {

    private ViewPager viewpagerPrivateTrip;
    private TabLayout tabIdicatorPrivateTrip;
    private LinearLayoutManager mLayoutManager;
    private SchedulesPrivateTripDetailAdapter schedulesAdapter;
    private AddonsPrivateTripPackageAdapter addonsAdapter;
    private PoliciesPrivateTripPackageAdapter policiesAdapter;

    private String tour_id = "";
    private boolean isClicked = false;
    private TourPackageItem tourDetail;
    private DateHelper dateHelper;
    private TourPackageImageAdapter adapter;
    private DiscussionAdapter mAdapterDiscussion;
    private PriceAdapter listPriceAdapter;
    private AppBarLayout appbar;

    private FloatingActionButton floating_menu;
    private RelativeLayout rlBack, rlMore;
    private ImageView iconDuration, ivIconDown, ivIconSchedule, ivIconAbout, ivIconItinerary, iviconAddons, ivProfileReview;
    private TextView tvTitleTour, tvPrice, tvTypeTour, textDuration, tvDuration, textLocation, tvLocation,
            ...
            tvViewSchedules, tvStatusSchedules, tvManageSchedules,
    ...
    private RecyclerView rvTags, recyclerview_list_price, rvSchedule, rvAddons, rvCustomPolicy, rvCategory, rvDiscussion;
    private View ComponentPriceList, compAppbar, compBtn, compTypeTour,
            CompDuration, CompLocation, ComponentInfo,
    ...
    private boolean expandAbout=false, expandItinerary=false, expandTermOfService=false, expandInsurance=false, expandRefundPolicy=false;
    private RelativeLayout container;
    private LinearLayout llContent, llButton, llError, llAddons, llAgeRetriction, llAditionalCost, llOtherPolicy;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_private_trip_package_detail);
        Initialization();
        ...
        ComponentPriceList.setOnClickListener(this);
    ...
    private void Initialization() {
        ...
        viewpagerPrivateTrip = findViewById(R.id.viewpagerPrivateTrip);
        tabIdicatorPrivateTrip = findViewById(R.id.tabIdicatorPrivateTrip);
        ...
        tvTypeTour.setBackground(getResources().getDrawable(R.drawable.custom_bg_radius_light_blue));
        tvTypeTour.setText(R.string.text_private_tour);

        ComponentInfo = findViewById(R.id.ComponentInfo);
        ...
        ComponentPriceList = findViewById(R.id.ComponentPriceList);
        ivIconDown = ComponentPriceList.findViewById(R.id.ivIconDown);
        tvPrice = ComponentPriceList.findViewById(R.id.tvPrice);
        recyclerview_list_price = findViewById(R.id.recyclerview_list_price);
...
//        });
        ...
        SetImageTabSlider(tourPackageItem.getMedias());
        SetListCategory(tourPackageItem.getCategories());
        SetListTags(tourPackageItem.getTags());
        SetListPrice(tourPackageItem.getPrices());
    ...
    private void SetImageTabSlider(List<MediasItem> medias){
        List<MediasItem> newArray = new ArrayList<>();
        for (int l = medias.size()-1; l >= 0; l--){
            newArray.add(medias.get(l));
        }
        tabIdicatorPrivateTrip.setSelectedTabIndicatorHeight(0);
        adapter = new TourPackageImageAdapter(this, newArray);
        viewpagerPrivateTrip.setAdapter(adapter);
        tabIdicatorPrivateTrip.setupWithViewPager(viewpagerPrivateTrip);
    }

    private void SetListCategory(String[] category) {
    ...
    private void SetListPrice(List<PricesItem> pricesItem) {
        List<Integer> listPrice = new ArrayList<>();
        for (int i = 0; i <pricesItem.size() ; i++) {
            int price = Integer.parseInt(pricesItem.get(i).getPrice());
            List<Integer> prices = Arrays.asList(price);
            for (int ii = 0; ii <prices.size() ; ii++) {
                listPrice.add(prices.get(ii));
            }
        }
        int priceMin = findMin(listPrice);
        NumberFormat formatter = new DecimalFormat("#,###");
        double price = Double.parseDouble(String.valueOf(priceMin));

        tvPrice.setText(String.valueOf(formatter.format(price)));
        recyclerview_list_price.setHasFixedSize(true);
        RecyclerView.LayoutManager layoutManagerPrice = new LinearLayoutManager(this);
        recyclerview_list_price.setLayoutManager(layoutManagerPrice);
        listPriceAdapter = new PriceAdapter(this, pricesItem);
        recyclerview_list_price.setAdapter(listPriceAdapter);
    }
    ...
    private void SetListSchedules(List<SchedulesItem> schedules) {
        ...
        schedulesAdapter = new SchedulesPrivateTripDetailAdapter(this, schedules);
        rvSchedule.setHasFixedSize(true);
        rvSchedule.setNestedScrollingEnabled(false);
        mLayoutManager = new LinearLayoutManager(this);
        rvSchedule.setAdapter(schedulesAdapter);
    ...
    private void SetAddons(List<AddonsItem> addons) {
        ...
        if (addons.size() > 0) {
            addonsAdapter = new AddonsPrivateTripPackageAdapter(this, addons);
    ...
    private void SetListOtherPolicies(TourPackageItem tourPackage, List<CustomPolicyItem> customPolicies) {
        ...
        policiesAdapter = new PoliciesPrivateTripPackageAdapter(this, customPolicies);
    ...
    private void SetlistDisccussion(List<DiscussionReplyItem> discussionReplyItem) {
        ...
        mAdapterDiscussion = new DiscussionAdapter(discussionReplyItem, this, tour_id);
...
//            }
        ...
        } else if (v == tvViewSchedules || v == tvManageSchedules) {

            Intent i = new Intent(this, PrivateTourScheduleActivity.class);
        ...
        } else if (v == ComponentPriceList){

            if (!isClicked){
                recyclerview_list_price.setVisibility(View.VISIBLE);
                ComponentInfo.setVisibility(View.VISIBLE);
                ivIconDown.setImageResource(R.drawable.icon_10dp_arrow_down);
                isClicked = true;
            } else {
                recyclerview_list_price.setVisibility(View.GONE);
            ...
            Intent i = new Intent(this, EditTripPackageActivity.class);
            i.putExtra("tour_id", tour_id);
            i.putExtra("pageHome", 4);
            i.putExtra("filter_price_low", filter_price_low);
            i.putExtra("filter_price_max", filter_price_max);
            ...
            popupMenu.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
                ...
                public boolean onMenuItemClick(MenuItem item) {
                    iViewDeleteTour iViewDeleteTour = PrivateTripPackageDetailActivity.this;
                    DeleteTourService deleteTourService = new DeleteTourService(PrivateTripPackageDetailActivity.this, iViewDeleteTour);
...
//            dialog.show();
    ...
    public void onBackPressed() {
        Intent i = new Intent(PrivateTripPackageDetailActivity.this, HomeActivity.class);
        i.putExtra("pageHome", 4);
        i.putExtra("filter_price_low", filter_price_low);
        i.putExtra("filter_price_max", filter_price_max);
        i.putExtra("filter_place", filter_place);
